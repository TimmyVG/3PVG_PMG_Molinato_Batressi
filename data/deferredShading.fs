...
void main() {
  ...
  vec3 light =  vec3(0);
    ...
    float bias = 0.0005;
        ...
        case 2:
            light = PointLight(FragPos,normal,Diffuse,Specular);
            shadow = ShadowCalculationPoint(0.00015,FragPos,normal,Diffuse,Specular);
        break;

        case 3:
            light = SpotLight(FragPos,normal,Diffuse,Specular);
            shadow = ShadowCalculationDefault(bias,FragPos,normal,Diffuse,Specular);
            break;
        case 4:
            light = AmbientLight();
            break;
