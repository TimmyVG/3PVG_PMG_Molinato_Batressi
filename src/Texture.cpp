#include <GL/glew.h>
#include "mew/Texture.hpp"

MEW::TextureData MEW::TextureFromFile(const char* path, const std::string& directory) {
 ...
 MEW::Texture::Texture(const TextureData& texture_data) {
	terminate_ = true;
...
MEW::Texture::Texture(Texture&& other) noexcept : id_(other.id_), terminate_(other.terminate_) {
  other.id_ = 0;
	other.terminate_ = false;
}

MEW::Texture& MEW::Texture::operator=(const Texture& other) {
	id_ = other.id_;
	terminate_ = other.terminate_;
	return *this;
}

MEW::Texture::Texture(Texture& other) {
	id_ = other.id_;
	terminate_ = other.terminate_;
}
